...
impl<S: SpartanExtensionField> ShiftProofs<S> {
    fn prove(
        ...
        let max_poly_size = shifted_polys
            .iter()
        ...
        for p in 0..num_instances {
            ...
            let orig_eval =
                (0..orig_poly.len()).fold(S::field_zero(), |a, b| a + orig_poly[b] * rc[b]);
            let shifted_eval =
...
impl<S: SpartanExtensionField> SNARK<S> {
    ...
    pub fn encode(inst: &Instance<S>) -> (ComputationCommitment<S>, ComputationDecommitment<S>) {
        ...
        timer_encode.stop();
        (ComputationCommitment { comm }, ComputationDecommitment {
            decomm,
        })
    }
        ...
        if total_num_mem_accesses > 0 {
            ...
            for q in (0..total_num_mem_accesses).rev() {
                ...
                if q != total_num_mem_accesses - 1 {
                    mem_w3[q][3] =
                        mem_w3[q][1] * (mem_w3[q + 1][2] + S::field_one() - mem_w3[q + 1][0]);
                } else {
    ...
    pub fn prove(
        ...
        let output: S = S::from_bytes(output).unwrap();
        {
            ...
            S::append_field_to_transcript(
                b"input_offset",
                transcript,
                S::from(input_offset as u64),
            );
            S::append_field_to_transcript(
                b"output_offset",
                transcript,
                S::from(output_offset as u64),
            );
            S::append_field_to_transcript(
                b"output_exec_num",
                transcript,
        ...
        let (
            ...
            let perm_exec_w2 = {
                ...
                for q in 0..consis_num_proofs {
                    ...
                    for i in 0..num_inputs_unpadded - 1 {
                        ...
                        perm_exec_w2[q][2] +=
                            perm * exec_inputs_list[q][2 + (num_inputs_unpadded - 1) + i];
                    }
                    perm_exec_w2[q][0] *= exec_inputs_list[q][0];
            ...
            let block_w2_prover = {
                ...
                let block_w2_size_list: Vec<usize> = (0..block_num_instances)
                    .map(|i| {
                        (2 * num_inputs_unpadded
                            + 2 * block_num_phy_ops[i]
                            + 4 * block_num_vir_ops[i])
                            .next_power_of_two()
                    })
                    .collect();
                ...
                let V_VMR2 = |b: usize, i: usize| {
                    2 * num_inputs_unpadded + 2 * block_num_phy_ops[b] + 4 * i + 1
                };
                let V_VMR3 = |b: usize, i: usize| {
                    2 * num_inputs_unpadded + 2 * block_num_phy_ops[b] + 4 * i + 2
                };
                let V_VMC = |b: usize, i: usize| {
                    2 * num_inputs_unpadded + 2 * block_num_phy_ops[b] + 4 * i + 3
                };
                ...
                for p in 0..block_num_instances {
                    ...
                    for q in (0..block_num_proofs[p]).rev() {
                        ...
                        for i in 0..num_inputs_unpadded - 1 {
                            ...
                            block_w2[p][q][2] +=
                                perm * block_vars_mat[p][q][2 + (num_inputs_unpadded - 1) + i];
                        }
                        block_w2[p][q][0] *= block_vars_mat[p][q][0];
                        ...
                        for i in 0..block_num_phy_ops[p] {
                            // PMR = r * PD
                            block_w2[p][q][V_PMR(i)] =
                                comb_r * block_vars_mat[p][q][io_width + V_PD(i)];
                            ...
                            block_w2[p][q][V_PMC(i)] = t
                                * (comb_tau
                                    - block_vars_mat[p][q][io_width + V_PA(i)]
                                    - block_w2[p][q][V_PMR(i)]);
                        }
                        // Compute x
                        ...
                        if q != block_num_proofs[p] - 1 {
                            block_w3[p][q][5] = px
                                * (block_w3[p][q + 1][4] + S::field_one() - block_w3[p][q + 1][0]);
                        } else {
                            block_w3[p][q][5] = px;
                        }
                        ...
                        for i in 0..block_num_vir_ops[p] {
                            ...
                            block_w2[p][q][V_VMR1(p, i)] =
                                comb_r * block_vars_mat[p][q][io_width + V_VD(p, i)];
                            // VMR2 = r^2 * VL
                            block_w2[p][q][V_VMR2(p, i)] =
                                comb_r * comb_r * block_vars_mat[p][q][io_width + V_VL(p, i)];
                            // VMR1 = r^3 * VT
                            block_w2[p][q][V_VMR3(p, i)] = comb_r
                                * comb_r
                                * comb_r
                                * block_vars_mat[p][q][io_width + V_VT(p, i)];
                        ...
                        if q != block_num_proofs[p] - 1 {
                            block_w3[p][q][7] = vx
                                * (block_w3[p][q + 1][6] + S::field_one() - block_w3[p][q + 1][0]);
                        } else {
                            block_w3[p][q][7] = vx;
                        }
        ...
        let (vir_mem_addr_w2_prover, vir_mem_addr_w3_prover, vir_mem_addr_w3_shifted_prover) = {
            if total_num_vir_mem_accesses > 0 {
                ...
                //       O = v * v = v
                // are used by (dummy) consistency check
                let mut vir_mem_addr_w3 =
                    vec![vec![S::field_zero(); W3_WIDTH]; total_num_vir_mem_accesses];
                for q in (0..total_num_vir_mem_accesses).rev() {
                    ...
                    if q != total_num_vir_mem_accesses - 1 {
                        vir_mem_addr_w3[q][3] = vir_mem_addr_w3[q][1]
                            * (vir_mem_addr_w3[q + 1][2] + S::field_one()
                                - vir_mem_addr_w3[q + 1][0]);
                    } else {
                        vir_mem_addr_w3[q][3] = vir_mem_addr_w3[q][1];
                    }
                ...
                let vir_mem_addr_w3_prover =
                    ProverWitnessSecInfo::new(vec![vir_mem_addr_w3.clone()], vec![
                        vir_mem_addr_poly_w3,
                    ]);
                let vir_mem_addr_w3_shifted_prover = ProverWitnessSecInfo::new(
                    vec![
                        [vir_mem_addr_w3[1..].to_vec(), vec![vec![
                            S::field_zero();
                            W3_WIDTH
                        ]]]
                        .concat(),
        ...
        let (block_poly_vars_list, exec_poly_inputs) = {
            ...
            for p in 0..block_num_instances {
                let block_poly_vars = {
                    // Flatten the witnesses into a Q_i * X list
                    let vars_list_p: Vec<S> =
                        block_vars_mat[p].clone().into_iter().flatten().collect();
                    // create a multilinear polynomial using the supplied assignment for variables
            ...
            if total_num_phy_mem_accesses > 0 {
                ...
                let addr_phy_mems_shifted_prover = {
                    ...
                    ProverWitnessSecInfo::new(
                        vec![
                            [addr_phy_mems_list[1..].to_vec(), vec![vec![
                                S::field_zero();
                                PHY_MEM_WIDTH
                            ]]]
                            .concat(),
        ...
        let (addr_poly_vir_mems, addr_vir_mems_shifted_prover, addr_ts_bits_prover) = {
            if total_num_vir_mem_accesses > 0 {
                ...
                let addr_vir_mems_shifted_prover = {
                    ...
                    ProverWitnessSecInfo::new(
                        vec![
                            [addr_vir_mems_list[1..].to_vec(), vec![vec![
                                S::field_zero();
                                VIR_MEM_WIDTH
                            ]]]
                            .concat(),
        ...
        let exec_inputs_prover =
            ProverWitnessSecInfo::new(vec![exec_inputs_list], exec_poly_inputs);
        ...
        let pairwise_num_proofs: Vec<usize> =
            pairwise_prover.w_mat.iter().map(|i| i.len()).collect();
        ...
        let (perm_root_inst_evals, perm_root_r1cs_eval_proof) = {
            ...
            let inst_evals = {
                ...
                for (val, tag) in
                    [(Ar, b"Ar_claim"), (Br, b"Br_claim"), (Cr, b"Cr_claim")].into_iter()
                {
    ...
    pub fn verify(
      ...
      bincode::serialize(output).unwrap().len();
        ...
        let (init_phy_mem_w2_verifier, init_phy_mem_w3_verifier, init_phy_mem_w3_shifted_verifier) = {
            if total_num_init_phy_mem_accesses > 0 {
                (
                    VerifierWitnessSecInfo::new(vec![INIT_PHY_MEM_WIDTH], &vec![
                        total_num_init_phy_mem_accesses,
                    ]),
                    VerifierWitnessSecInfo::new(vec![W3_WIDTH], &vec![
                        total_num_init_phy_mem_accesses,
                    ]),
                    VerifierWitnessSecInfo::new(vec![W3_WIDTH], &vec![
                        total_num_init_phy_mem_accesses,
        ...
        let (init_vir_mem_w2_verifier, init_vir_mem_w3_verifier, init_vir_mem_w3_shifted_verifier) = {
            if total_num_init_vir_mem_accesses > 0 {
                (
                    VerifierWitnessSecInfo::new(vec![INIT_VIR_MEM_WIDTH], &vec![
                        total_num_init_vir_mem_accesses,
                    ]),
                    VerifierWitnessSecInfo::new(vec![W3_WIDTH], &vec![
                        total_num_init_vir_mem_accesses,
        ...
        let (phy_mem_addr_w2_verifier, phy_mem_addr_w3_verifier, phy_mem_addr_w3_shifted_verifier) = {
            if total_num_phy_mem_accesses > 0 {
                (
                    VerifierWitnessSecInfo::new(vec![PHY_MEM_WIDTH], &vec![
                        total_num_phy_mem_accesses,
                    ]),
        ...
        let (vir_mem_addr_w2_verifier, vir_mem_addr_w3_verifier, vir_mem_addr_w3_shifted_verifier) = {
            if total_num_vir_mem_accesses > 0 {
                (
                    VerifierWitnessSecInfo::new(vec![VIR_MEM_WIDTH], &vec![
                        total_num_vir_mem_accesses,
                    ]),
        ...
        let (addr_vir_mems_verifier, addr_vir_mems_shifted_verifier, addr_ts_bits_verifier) = {
            if total_num_vir_mem_accesses > 0 {
                (
                    ...
                    VerifierWitnessSecInfo::new(vec![mem_addr_ts_bits_size], &vec![
                        total_num_vir_mem_accesses,
                    ]),
                )
            } else {
