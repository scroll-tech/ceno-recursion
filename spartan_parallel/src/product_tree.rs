...
use super::transcript::{Transcript, append_field_to_transcript, challenge_scalar, challenge_vector};
...
impl<E: ExtensionField> ProductCircuitEvalProof<E> {
  ...
  pub fn prove(circuit: &mut ProductCircuit<E>, transcript: &mut Transcript<E>) -> (Self, E, Vec<E>) {
    ...
    for layer_id in (0..num_layers).rev() {
      ...
      append_field_to_transcript(transcript, claims_prod[0]);
      append_field_to_transcript(transcript, claims_prod[1]);
  ...
  pub fn prove(
    ...
    for layer_id in (0..num_layers).rev() {
      ...
      for i in 0..prod_circuit_vec.len() {
        append_field_to_transcript(transcript, claims_prod_left[i]);
        append_field_to_transcript(transcript, claims_prod_right[i]);
      }

      if layer_id == 0 && !dotp_circuit_vec.is_empty() {
        let (claims_dotp_left, claims_dotp_right, claims_dotp_weight) = claims_dotp;
        for i in 0..dotp_circuit_vec.len() {
          append_field_to_transcript(transcript, claims_dotp_left[i]);
          append_field_to_transcript(transcript, claims_dotp_right[i]);
          append_field_to_transcript(transcript, claims_dotp_weight[i]);
        }
