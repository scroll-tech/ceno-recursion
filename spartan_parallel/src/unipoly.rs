use crate::transcript::{Transcript, append_message, append_field_vector_to_transcript};
...
impl<E: ExtensionField> UniPoly<E> {
  ...
  pub fn append_to_transcript(&self, label: &'static [u8], transcript: &mut Transcript<E>) {
    append_message(transcript, b"UniPoly_begin");
    append_field_vector_to_transcript(transcript, &self.coeffs);
    append_message(transcript, b"UniPoly_end");
  }
}
