...
impl<'ast> ZGen<'ast> {
    ...
    fn int_to_t(&self, int: &Integer, ty: &Ty) -> Result<T, String> {
        let e = &(LiteralExpression::DecimalLiteral(DecimalLiteralExpression {
            value: DecimalNumber {
                value: int.to_string(),
    ...
    > {
        ...
        let prog_inputs = &[
            vec![
                ("%BN".to_string(), Ty::Field),
                ("%SP".to_string(), Ty::Field),
                ("%AS".to_string(), Ty::Field),
            ],
            prog_inputs.clone(),
        ]
        .concat();
            ...
            match x {
                Ty::Uint(_) | Ty::Field | Ty::Bool => {
                    if *alive {
                        ...
                        self.declare_init_impl_::<true>(name.to_string(), x.clone(), val)?;
                        input_count += 1;
                    }
                }
                Ty::Array(read_only, _, entry_ty) => {
                    let entry_ty = match **entry_ty {
                        ...
                        Ty::Array(..) => &Ty::Field,
                        _ => {
                            panic!("Struct input type not supported!")
                        }
                    };
                    if *alive {
                        let (name, _) = &bls[entry_bl].inputs[input_count];
                        ...
                        self.declare_init_impl_::<true>(name.to_string(), Ty::Field, val)?;
                        input_count += 1;
                    }
                    ...
                    if *read_only {
                        ...
                        for entry in &entry_stacks[i] {
                            ...
                            let addr_t =
                                self.int_to_t(&Integer::from(stack_addr_count), &Ty::Field)?;
                        ...
                        for entry in &entry_arrays[i] {
                            ...
                            let addr_t =
                                self.int_to_t(&Integer::from(mem_addr_count), &Ty::Field)?;
                            let data_t = self.int_to_t(&entry, &*entry_ty)?;
                            ...
                            init_vir_mem_list
                                .push(MemOp::new_vir(addr, addr_t, data_t, ls_t, ts, ts_t));
                            mem_addr_count += 1;
                        }
                    }
                }
        ...
        while !terminated {
            ...
            // Record the transition state
                ...
                for (name, ty) in &bls[nb].inputs {
                    if let Some(x) = ty {
                        ...
                        let val = self.cvar_lookup(&output_name).unwrap_or_else(|| {
                            self.expr_impl_::<true>(&Expression::Literal(match x {
                                Ty::Bool => {
                                ...
                                _ => LiteralExpression::DecimalLiteral(DecimalLiteralExpression {
                                    value: DecimalNumber {
                                        value: "0".to_string(),
                                        span: Span::new("", 0, 0).unwrap(),
                                    },
                ...
                for i in 1..io_size {
                    bl_exec_state[tr_size - 1].reg_out[i] =
                        self.cvar_lookup(&format!("%o{:06}", i));
                    if bl_exec_state[tr_size - 1].reg_out[i].is_none() {
                        bl_exec_state[tr_size - 1].reg_out[i] =
                            self.cvar_lookup(&format!("%i{:06}", i));
                    }
                }
            ...
            (
                ...
                wit_op,
                witness_count,
            ) = self.bl_eval_impl_(&bls[nb], phy_mem, vir_mem, entry_witnesses, witness_count)?;
        ...
        let ret = self
            .cvar_lookup(O_RET)
            .ok_or(format!("Missing return value for one or more functions."));

        let (phy_mem_list, vir_mem_list) =
            sort_by_mem(&init_phy_mem_list, &init_vir_mem_list, &bl_exec_state);
        Ok((
            ret?,
            bl_exec_count,
            prog_reg_in,
        ...
        match &bl.terminator {
            BlockTerminator::Transition(e) => {
                match self.t_to_usize(self.expr_impl_::<true>(&e)?) {
                    ...
                    _ => {
                        return Err("Evaluation failed: block transition evaluated to an invalid block label".to_string());
                    }
                }
            }
            BlockTerminator::FuncCall(fc) => Err(format!(
                "Evaluation failed: function call to {} needs to be converted to block label.",
                fc
            )),
            BlockTerminator::ProgTerm => Ok((
                0,
                phy_mem,
                vir_mem,
                true,
        ...
        for s in inst {
            ...
            match s {
                BlockContent::Witness((var, ty, alive)) => {
                    if *alive {
                        let ty = if let Ty::Array(..) = ty {
                            &Ty::Field
                        } else {
                            ty
                        };
                        let val = self.int_to_t(&entry_witnesses[witness_count], &ty)?;
                        self.declare_init_impl_::<true>(var.to_string(), ty.clone(), val.clone())?;
                        wit_op.push(val);
                    }
                    witness_count += 1;
                }
                ...
                BlockContent::MemPop((var, _, offset)) => {
                    let bp_t = self
                        .cvar_lookup(W_BP)
                        .ok_or(format!("Pop from %PHY failed: %BP is uninitialized."))?;
                ...
                BlockContent::ArrayInit((arr, _, len_expr, read_only)) => {
                    ...
                    let pointer_t = if *read_only {
                        self.cvar_lookup(W_SP).ok_or(format!(
                            "Read-only array initialization failed: %SP is uninitialized."
                        ))?
                    } else {
                        self.cvar_lookup(W_AS).ok_or(format!(
                            "Array initialization failed: %AS is uninitialized."
                        ))?
                    };
                    self.declare_init_impl_::<true>(arr.to_string(), Ty::Field, pointer_t.clone())?;
                ...
                BlockContent::Store((val_expr, _, arr, id_expr, init, read_only)) => {
                    ...
                    let offset_t = self
                        .cvar_lookup(arr)
                        .ok_or(format!("Store failed: array {} is uninitialized.", arr))?;
                    ...
                    if *read_only {
                        ...
                        ro_mem_op.push(MemOp::new_phy(addr, addr_t, val_t));
                    } else {
                        if addr >= vir_mem.len() {
                        ...
                        // Update vir_mem_op
                        let ls_t = self
                            .expr_impl_::<false>(&Expression::Literal(
                        ...
                        // %TS = %TS + 1
                        if !init {
                            self.bl_eval_stmt_impl_(&bl_gen_increment_stmt(W_TS, 1, &Ty::Field))
                                .unwrap();
                        }
                        let ts_t = self
                            .cvar_lookup(W_TS)
                            .ok_or(format!("STORE failed: %TS is uninitialized."))?;
                        ...
                        vir_mem_op.push(MemOp::new_vir(addr, addr_t, val_t, ls_t, ts, ts_t));
                    }
                }
                BlockContent::Load((var, ty, arr, id_expr, read_only)) => {
                    ...
                    let mut val_t = if *read_only {
                        phy_mem[addr].clone().ok_or(format!(
                            "Read-only LOAD failed: entry {} is uninitialized.",
                            addr
                        ))?
                    } else {
                        vir_mem[addr]
                            .clone()
                            .ok_or(format!("LOAD failed: entry {} is uninitialized.", addr))?
                    };
    ...
    fn bl_eval_stmt_impl_(&self, s: &Statement) -> Result<(), String> {
        match s {
            Statement::Return(_) => {
