...
use crate::{dense_mlpoly::DensePolynomial, scalar::SpartanExtensionField};
                    ...
                    for w in 0..min(self.num_witness_secs, self.Z[p][q].len()) {
                        for x in 0..self.num_inputs[p] {
                            self.Z[p][q][w][x] = self.Z[p][q][w][x]
                                + *r * (self.Z[p][q + self.num_proofs[p]][w][x]
                                    - self.Z[p][q][w][x]);
                        }
                    }
                }
            ...
            for q in 0..self.num_proofs[p] {
                for w in 0..self.num_witness_secs {
                    for x in 0..self.num_inputs[p] {
                        ...
                        self.Z[p][q][w][x] =
                            self.Z[p][q][w][x] + *r * (Z_high - self.Z[p][q][w][x]);
                    }
                }
            }
        }
            ...
            if self.num_inputs[p] == 1 {
                ...
                for q in 0..self.num_proofs[p] {
                    for w in 0..min(self.num_witness_secs, self.Z[p][q].len()) {
                        for x in 0..self.num_inputs[p] {
                            self.Z[p][q][w][x] = self.Z[p][q][w][x]
                                + *r * (self.Z[p][q][w][x + self.num_inputs[p]]
                                    - self.Z[p][q][w][x]);
                        }
                    }
                }
