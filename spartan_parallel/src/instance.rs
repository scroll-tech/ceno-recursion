...
use crate::{R1CSInstance, errors::R1CSError, math::Math, scalar::SpartanExtensionField};
...
impl<S: SpartanExtensionField> Instance<S> {
    ...
    pub fn new(
        ...
        let bytes_to_scalar = |b: usize,
         ...
         -> Result<Vec<(usize, usize, S)>, R1CSError> {
    ...
    pub fn gen_block_inst<const PRINT_SIZE: bool>(
        ...
        let V_VMR1 = |b: usize, i: usize| {
            2 * num_vars + 2 * num_inputs_unpadded + 2 * num_phy_ops[b] + 4 * i
        };
        let V_VMR2 = |b: usize, i: usize| {
            2 * num_vars + 2 * num_inputs_unpadded + 2 * num_phy_ops[b] + 4 * i + 1
        };
        let V_VMR3 = |b: usize, i: usize| {
            2 * num_vars + 2 * num_inputs_unpadded + 2 * num_phy_ops[b] + 4 * i + 2
        };
        let V_VMC = |b: usize, i: usize| {
            2 * num_vars + 2 * num_inputs_unpadded + 2 * num_phy_ops[b] + 4 * i + 3
        };
        // in BLOCK_W3
        let V_v = 3 * num_vars;
        ...
        for b in 0..num_instances {
            ...
            let (A, B, C) = {
                ...
                // constraints for input permutation
                {
                    ...
                    // v[k]
                    (A, B, C) = Instance::<S>::gen_constr(A, B, C, counter, vec![], vec![], vec![
                        (V_valid, 1),
                        (V_v, -1),
                    ]);
                    counter += 1;
    ...
    pub fn gen_pairwise_check_inst<const PRINT_SIZE: bool>(
        ...
        let pairwise_check_num_non_zero_entries: usize =
            max(13 + max_ts_width, 5 + 2 * max_ts_width);
    ...
    pub fn gen_perm_root_inst<const PRINT_SIZE: bool>(
        ...
        let perm_root_inst = {
            let (A, B, C) = {
                ...
                // v[k]
                (A, B, C) =
                    Instance::<S>::gen_constr(A, B, C, constraint_count, vec![], vec![], vec![
                        (V_valid, 1),
                        (V_v, -1),
                    ]);
                constraint_count += 1;
