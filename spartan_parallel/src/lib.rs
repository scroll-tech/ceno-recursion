...
impl<S: SpartanExtensionField + 'static> SNARK<S> {
  ...
  pub fn prove(
    ...
    let (block_inst_evals_bound_rp, block_inst_evals_list, block_r1cs_eval_proof_list) = {
      ...
      let (_, inst_evals_bound_rp) =
        R1CSInstance::multi_evaluate_bound_rp::<true>(arc_block_inst.as_ref(), &rp, &rx, &ry);
      timer_eval.stop();
    ...
    let (
      ...
      // RP-bound evaluation is sorted
      let (_, inst_evals_bound_rp) = R1CSInstance::multi_evaluate_bound_rp::<false>(
        arc_pairwise_check_inst.as_ref(),
        &rp,
        &rx,
        &ry,
      );
