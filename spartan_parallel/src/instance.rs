...
impl<E: ExtensionField> Instance<E> {
    ...
    let bytes_to_scalar =
      |b: usize, tups: &[(usize, usize, [u8; 32])]| -> Result<Vec<(usize, usize, E)>, R1CSError> {
        ...
        for &(row, col, val_bytes) in tups {
          ...
          let val: CtOption<E> = from_bytes(&val_bytes);
  ...
  pub fn gen_pairwise_check_inst<const PRINT_SIZE: bool>(
    ...
    let pairwise_check_inst = {
      ...
      let pairwise_check_inst = Instance::<E>::new(
        3,
  ...
  pub fn gen_perm_root_inst<const PRINT_SIZE: bool>(
    ...
    let perm_root_inst = {
      ...
      let perm_root_inst = Instance::<E>::new(
        1,
        perm_root_num_cons,
