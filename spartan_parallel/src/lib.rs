...
use transcript::{Transcript, append_protocol_name, append_field_to_transcript, append_field_vector_to_transcript, challenge_scalar, challenge_vector};
...
impl<'a, E: ExtensionField + Send + Sync> SNARK<E> {
  ...
  pub fn prove(
    ...
    let (block_inst_evals_bound_rp, block_inst_evals_list, block_r1cs_eval_proof_list) = {
      let [rp, _, rx, ry]: [Vec<E>; 4] = block_challenges;
    ...
    let (
      ...
      let [rp, _, rx, ry]: [Vec<E>; 4] = pairwise_check_challenges;
      let timer_eval = Timer::new("eval_sparse_polys");
    ...
    let (perm_root_inst_evals, perm_root_r1cs_eval_proof) = {
      let [_, _, rx, ry]: [Vec<E>; 4] = perm_root_challenges;
