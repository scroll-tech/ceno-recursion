...
/// q = 2^64 - 2^32 + 1
/// 0x0000000000000000 0000000000000000 0000000000000000 FFFFFFFF00000001
...
impl Scalar {
  ...
  fn from_u512(limbs: [u64; 8]) -> Scalar {
    ...
    // 2. the upper bits are multiplied by R^2 * 2^256 = 2^384
    //
    // and computing their sum in the field. It remains to see that arbitrary 256-bit
    // numbers can be placed into Montgomery form safely using the reduction. The
    // reduction works so long as the product is less than R=2^64 multipled by
  ...
  pub fn invert(&self) -> CtOption<Self> {
    let a = u64::from_le_bytes(<[u8; 8]>::try_from(&self.to_bytes()[..8]).unwrap());
    let a_inv = mod_inverse(a, P);
    CtOption::new(a_inv.into(), !self.ct_eq(&Self::zero()))
  }
...
fn mod_inverse(a: u64, p: u64) -> u64 {
  ...
  (x % p + p) % p  // Ensure the result is positive
}
