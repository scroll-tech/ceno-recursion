...
impl<'ast> ZGen<'ast> {
    ...
    fn bl_gen_function_call_(
        ...
        let mut scope_phy_assign: Vec<(usize, String)> = Vec::new();
        ...
        if self.stdlib.is_embed(&f_path) {
            ...
            for (p, a) in f.parameters.clone().into_iter().zip(args) {
                ...
                (blks, scope_phy_assign, sp_offset) = self.bl_gen_scoping_(blks, blks_len, &p.id.value, scope_phy_assign, sp_offset);
                // Assign p to a
            ...
            (blks, scope_phy_assign, sp_offset) = 
                self.bl_gen_scoping_(blks, blks_len, &"%RP".to_string(), scope_phy_assign, sp_offset);
        ...
        match e {
            ...
            Expression::Postfix(p) => {
                ...
                if let Some(Access::Call(c)) = p.accesses.first() {
                    ...
                    (blks, blks_len, sp_offset) = self.bl_gen_exit_scope_(blks, blks_len, scope_phy_assign.clone(), sp_offset)?;
