...
impl<S: SpartanExtensionField> DensePolynomialPqx<S> {
  ...
  pub fn new(z_mat: &Vec<Vec<Vec<Vec<S>>>>, num_proofs: Vec<usize>, max_num_proofs: usize, num_inputs: Vec<usize>, max_num_inputs: usize) -> Self {
  ...
  pub fn new_rev(z_mat: &Vec<Vec<Vec<Vec<S>>>>, num_proofs: Vec<usize>, max_num_proofs: usize, num_inputs: Vec<usize>, max_num_inputs: usize) -> Self {
    let mut Z = Vec::new();
    let num_instances = z_mat.len();
    let num_witness_secs = z_mat[0][0].len();
    for p in 0..num_instances {
      Z.push(vec![vec![vec![S::field_zero(); num_inputs[p]]; num_witness_secs]; num_proofs[p]]);
  ...
  pub fn index_high(&self, p: usize, q_rev: usize, w: usize, x_rev: usize, mode: usize) -> S {
      match mode {
          MODE_P => { if p + self.num_instances / 2 < self.Z.len() { return self.Z[p + self.num_instances / 2][q_rev][w][x_rev] } else { return S::field_zero(); } }
          MODE_Q => { return if self.num_proofs[p] == 1 { S::field_zero() } else { self.Z[p][q_rev + self.num_proofs[p] / 2][w][x_rev] }; }
          MODE_W => { if w + self.num_witness_secs / 2 < self.Z[p][q_rev].len() { return self.Z[p][q_rev][w + self.num_witness_secs / 2][x_rev] } else { return S::field_zero(); } }
          MODE_X => { return if self.num_inputs[p] == 1 { S::field_zero() } else { self.Z[p][q_rev][w][x_rev + self.num_inputs[p] / 2] }; }
  ...
  pub fn bound_poly_p(&mut self, r: &S) {
      ...
      for p in 0..self.num_instances {
          for w in 0..min(self.num_witness_secs, self.Z[p][0].len()) {
              let Z_high = if p + self.num_instances < self.Z.len() { self.Z[p + self.num_instances][0][w][0] } else { S::field_zero() };
        ...
        if self.num_proofs[p] == 1 {
          ...
          for q in 0..self.num_proofs[p] {
            for w in 0..min(self.num_witness_secs, self.Z[p][q].len()) {
              for x in 0..self.num_inputs[p] {
                self.Z[p][q][w][x] = self.Z[p][q][w][x] + *r * (self.Z[p][q + self.num_proofs[p]][w][x] - self.Z[p][q][w][x]);
              }
            }
          }
        }
    ...
    for p in 0..min(self.num_instances, self.Z.len()) {
      for q in 0..self.num_proofs[p] {
        for w in 0..self.num_witness_secs {
          for x in 0..self.num_inputs[p] {
            let Z_high = if w + self.num_witness_secs < self.Z[p][q].len() { self.Z[p][q][w + self.num_witness_secs][x] } else { S::field_zero() };
      ...
      for p in 0..min(self.num_instances, self.Z.len()) {
        if self.num_inputs[p] == 1 {
          ...
          for q in 0..self.num_proofs[p] {
            for w in 0..min(self.num_witness_secs, self.Z[p][q].len()) {
              for x in 0..self.num_inputs[p] {
                  self.Z[p][q][w][x] = self.Z[p][q][w][x] + *r * (self.Z[p][q][w][x + self.num_inputs[p]] - self.Z[p][q][w][x]);
              }
            }
          }
        }
  ...
  pub fn to_dense_poly(&self) -> DensePolynomial<S> {
      let mut Z_poly = vec![S::field_zero(); self.num_instances * self.max_num_proofs * self.num_witness_secs * self.max_num_inputs];
      for p in 0..min(self.num_instances, self.Z.len()) {
