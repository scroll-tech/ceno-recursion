...
use super::transcript::{Transcript, challenge_vector, challenge_scalar, append_protocol_name, append_field_to_transcript, append_field_vector_to_transcript};
...
impl<E: ExtensionField> SparseMatPolyCommitment<E> {
  pub fn append_to_transcript(&self, _label: &'static [u8], transcript: &mut Transcript<E>) {
    ...
    append_field_to_transcript(b"num_mem_cells", transcript, E::from(self.num_mem_cells as u64));
  }
}
...
impl<E: ExtensionField> ProductLayerProof<E> {
  ...
  pub fn prove(
    ...
    append_field_to_transcript(b"claim_row_eval_init", transcript, row_eval_init);
    append_field_vector_to_transcript(b"claim_row_eval_read", transcript, &row_eval_read);
    append_field_vector_to_transcript(b"claim_row_eval_write", transcript, &row_eval_write);
    append_field_to_transcript(b"claim_row_eval_audit", transcript, row_eval_audit);
    ...
    append_field_to_transcript(b"claim_col_eval_init", transcript, col_eval_init);
    append_field_vector_to_transcript(b"claim_col_eval_read", transcript, &col_eval_read);
    append_field_vector_to_transcript(b"claim_col_eval_write", transcript, &col_eval_write);
    append_field_to_transcript(b"claim_col_eval_audit", transcript, col_eval_audit);
    ...
    for i in 0..derefs.row_ops_val.len() {
      ...
      append_field_to_transcript(b"claim_eval_dotp_left", transcript, eval_dotp_left);
      append_field_to_transcript(b"claim_eval_dotp_right", transcript, eval_dotp_right);
  ...
  pub fn verify(
    ...
    for i in 0..num_instances {
      ...
      append_field_to_transcript(b"claim_eval_dotp_left", transcript, eval_dotp_left[i]);
      append_field_to_transcript(b"claim_eval_dotp_right", transcript, eval_dotp_right[i]);
