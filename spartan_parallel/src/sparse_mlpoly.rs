    ...
    let poly_init_hashed = DensePolynomial::new(
      (0..num_mem_cells)
        .map(|i| {
          ...
          hash_func(&S::from(i as u64), &eval_table[i], &S::field_zero()) - *r_multiset_check
        })
        .collect::<Vec<S>>(),
    );
      ...
      let poly_write_hashed = DensePolynomial::new(
        (0..num_ops)
          .map(|i| {
            ...
            hash_func(&addrs[i], &derefs[i], &(read_ts[i] + S::field_one())) - *r_multiset_check
          })
          .collect::<Vec<S>>(),
      );
...
impl<S: SpartanExtensionField> HashLayerProof<S> {
  ...
  fn verify(
    ...
    for i in 0..claims_dotp.len() / 3 {
      ...
      /* TODO: IMPORTANT, DEBUG, CHECK FAIL*/
      assert_eq!(
        claim_row_ops_val, eval_row_ops_val[i],
        "Failed at i = {}",
        i
      );
