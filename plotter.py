            ...
            if case_name in circ_b_name_list:
                ...
                for j in range(3):
                    ...
                    if time_entries[0][k] == 0:
                        ...
                        circ_data[j].append([time_entries[0][k] / time_entries[1][k], time_entries[0][k] / time_entries[4][k], time_entries[0][k] / time_entries[5][k]])
...
def gen_circ_jolt_plots(circ_benchmark_names, circ_data, jolt_benchmark_names, jolt_data):
    ...
    # CIRC
    runtime_subplot_name = ["Compiler Speedup (from CirC)", "Prover Speedup (from CirC)", "Verifier Speedup (from CirC)"]
...
def gen_proof_size_plot(proof_size_benchmark_names, proof_size_data):
    ...
    for bench in range(len(proof_size_benchmark_names) - 1):
        ...
        for j in range(3):
            ...
            axi.annotate((f"%0.0fkB" % (proof_size_data[bench][j] / 1000)), (5 * bench + j, proof_size_data[bench][j]), horizontalalignment='center', verticalalignment='bottom')
    ...
    for j in range(3):
        ...
        axi.annotate((f"%0.0fkB" % (proof_size_data[bench][j] / 1000)), (j, proof_size_data[bench][j]), horizontalalignment='center', verticalalignment='bottom')
    axi.set_xticks([0.5])
    axi.set_xticklabels(proof_size_benchmark_names[-1:])
