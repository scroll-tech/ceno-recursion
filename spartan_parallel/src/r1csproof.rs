...
use super::{
    custom_dense_mlpoly::DensePolynomialPqx,
...
use crate::{ProverWitnessSecInfo, VerifierWitnessSecInfo, scalar::SpartanExtensionField};
...
impl<S: SpartanExtensionField> R1CSProof<S> {
            ...
            for p in 0..wit_sec_num_instance {
                ...
                let ry_short = {
                    ...
                    if w.num_inputs[p] >= max_num_inputs {
                        let ry_pad =
                            vec![S::field_zero(); w.num_inputs[p].log_2() - max_num_inputs.log_2()];
                ...
                let rq_short = rq
                    [num_rounds_q - num_proofs_list[num_proofs_list.len() - 1].log_2()..]
                    .to_vec();
    ...
    pub fn verify(
        ...
        let taus_bound_rp: S = (0..rp_round1.len())
            .map(|i| {
                rp_round1[i] * tau_p[i]
                    + (S::field_one() - rp_round1[i]) * (S::field_one() - tau_p[i])
            })
            .product();
        let taus_bound_rq: S = (0..rq_rev.len())
            .map(|i| {
                rq_rev[i] * tau_q[i] + (S::field_one() - rq_rev[i]) * (S::field_one() - tau_q[i])
            })
            .product();
        let taus_bound_rx: S = (0..rx_rev.len())
            .map(|i| {
                rx_rev[i] * tau_x[i] + (S::field_one() - rx_rev[i]) * (S::field_one() - tau_x[i])
            })
            .product();
        ...
        let _p_rp_poly_bound_ry: S = (0..rp.len())
            .map(|i| {
                rp[i] * rp_round1[i] + (S::field_one() - rp[i]) * (S::field_one() - rp_round1[i])
            })
            .product();
